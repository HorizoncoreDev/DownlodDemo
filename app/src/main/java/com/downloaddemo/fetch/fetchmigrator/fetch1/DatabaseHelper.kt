...
internal class DatabaseHelper constructor(context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL(
            "CREATE TABLE $TABLE_NAME ( $COLUMN_ID INTEGER PRIMARY KEY NOT NULL, $COLUMN_URL TEXT NOT NULL, $COLUMN_FILEPATH TEXT NOT NULL, $COLUMN_STATUS INTEGER NOT NULL, $COLUMN_HEADERS TEXT NOT NULL, $COLUMN_DOWNLOADED_BYTES INTEGER NOT NULL, $COLUMN_FILE_SIZE INTEGER NOT NULL, $COLUMN_ERROR INTEGER NOT NULL, $COLUMN_PRIORITY INTEGER NOT NULL, unique( $COLUMN_FILEPATH ) )"
        )
    }
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        when (oldVersion) {
            1 -> {
                db.execSQL(
                    "CREATE UNIQUE INDEX table_unique ON $TABLE_NAME ( $COLUMN_FILEPATH)"
                )
            }
    ...
    fun verifyOK() {
        synchronized(lock) {
            db.beginTransaction()
            db.execSQL(
                "UPDATE $TABLE_NAME SET $COLUMN_STATUS = ? WHERE $COLUMN_STATUS = ?",
    ...
    fun clean() {
        synchronized(lock) {
            val cursor = db.rawQuery(
                "SELECT $COLUMN_ID, $COLUMN_FILEPATH FROM $TABLE_NAME WHERE $COLUMN_STATUS = ?",
            ...
            while (!cursor.isAfterLast) {
                val destinationUri = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_FILEPATH))
                if (destinationUri != null) {
                    if (!File(destinationUri).exists()) {
                        val id = cursor.getLong(cursor.getColumnIndexOrThrow(COLUMN_ID))
                        db.execSQL(
                            "UPDATE " + TABLE_NAME + " SET " + COLUMN_STATUS + " = ?," + FetchConst.STATUS_ERROR + ", " + COLUMN_ERROR + " = ? " + "WHERE " + COLUMN_ID + " = ?",
