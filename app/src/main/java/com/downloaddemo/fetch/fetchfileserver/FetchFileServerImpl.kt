...
import com.downloaddemo.fetch.fetchcore.getFileMd5String
...
import com.downloaddemo.fetch.fetchfileserver.database.FetchFileResourceInfoDatabase
import com.downloaddemo.fetch.fetchfileserver.database.FileResourceInfo
import com.downloaddemo.fetch.fetchfileserver.database.toFileResource
import com.downloaddemo.fetch.fetchfileserver.database.toFileResourceInfo
import com.downloaddemo.fetch.fetchfileserver.provider.FetchFileResourceProvider
import com.downloaddemo.fetch.fetchfileserver.provider.FileResourceProvider
import com.downloaddemo.fetch.fetchfileserver.provider.FileResourceProviderDelegate
...
class FetchFileServerImpl(
    ...
    private val fileResourceProviderMap =
        Collections.synchronizedMap(mutableMapOf<String, FileResourceProvider>())

    @Volatile
    private var isTerminated = false
    private var isForcedTerminated = false
    private var isStarted = false
    private val fileResourceServerDatabase =
        FetchFileResourceInfoDatabase(context.applicationContext, databaseName)
    private val ioHandler = run {
    ...
    override val address: String
        get() {
            return if (!isTerminated) {
                if (serverSocket.inetAddress.hostAddress != null) {
    ...
    private val fileResourceProviderDelegate = object : FileResourceProviderDelegate {
        ...
        override fun acceptAuthorization(
        ...
        override fun onClientDidProvideExtras(
            sessionId: String, extras: Extras, fileRequest: FileRequest
        ) {
            mainHandler.post {
        ...
        override fun getFileInputResourceWrapper(
            sessionId: String,
            fileRequest: FileRequest,
            fileResource: FileResource,
            fileOffset: Long
        ): InputResourceWrapper? {
            return fetchFileServerDelegate?.getFileInputResourceWrapper(
                sessionId, fileRequest, fileResource, fileOffset
            )
        }

        override fun onStarted(
            sessionId: String, fileRequest: FileRequest, fileResource: FileResource
        ) {
            mainHandler.post {
        ...
        override fun onCustomRequest(
            sessionId: String,
            fileRequest: FileRequest,
            fileResourceTransporterWriter: FileResourceTransporterWriter,
            interruptMonitor: InterruptMonitor
        ) {
            fetchFileServerDelegate?.onCustomRequest(
                sessionId, fileRequest, fileResourceTransporterWriter, interruptMonitor
            )
        }
    ...
    private fun throwIfAddingReservedCatalogInfo(fileResource: FileResource) {
        if (fileResource.id == FileRequest.CATALOG_ID || fileResource.name == FileRequest.CATALOG_NAME || fileResource.file == FileRequest.CATALOG_FILE) {
            throw IllegalArgumentException(
                "File Resources 'id' cannot be: ${FileRequest.CATALOG_ID} " + "and 'name' cannot be: Catalog.json and " + "'file' cannot be: /Catalog.json"
            )
        }
    }
