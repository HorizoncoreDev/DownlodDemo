...
class FetchFileResourceInfoDatabase(
    ...
    private val fileResourceInfoDatabase = Room.databaseBuilder(
        context, FileResourceInfoDatabase::class.java, databaseName
    ).build()
        ...
        synchronized(lock) {
            ...
            fileResources.forEachIndexed { index, fileResource ->
                stringBuilder.append(
                    "{\"id\":${fileResource.id},\"name\":\"${fileResource.name}\"," + "\"length\":${fileResource.length},\"extras\":${fileResource.extras}," + "\"md5\":\"${fileResource.md5}\"}"
                )
