...
abstract class FileResolver(context: Context) {
     ...
     * */
    open fun getCatalogInputWrapper(
        catalog: ByteArray, request: FileRequest, fileResource: FileResource
    ): InputResourceWrapper {
        return object : InputResourceWrapper() {

            private val inputStream = ByteArrayInputStream(
                catalog, request.rangeStart.toInt(), fileResource.length.toInt()
            )
        ...
        return when (fileUri.scheme) {
            "content" -> {
                ...
                if (parcelFileDescriptor == null) {
                    ...
                    createUriInputResourceWrapper(
                        FileInputStream(parcelFileDescriptor.fileDescriptor), parcelFileDescriptor
                    )
                }
            }
    ...
    private fun createUriInputResourceWrapper(
        fileInputStream: FileInputStream, fileDescriptor: ParcelFileDescriptor? = null
    ): InputResourceWrapper {
        return object : InputResourceWrapper() {
    ...
    private fun getFileInputResourceWrapper(fileResource: FileResource): InputResourceWrapper {
        return object : InputResourceWrapper() {
            ...
            val randomAccessFile = RandomAccessFile(fileResource.file, "rw")
