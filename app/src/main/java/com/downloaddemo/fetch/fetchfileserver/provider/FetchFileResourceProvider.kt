...
import com.downloaddemo.fetch.fetchcore.calculateProgress
import com.downloaddemo.fetch.fetchcore.getMd5String
import com.downloaddemo.fetch.fetchcore.hasIntervalTimeElapsed
import com.downloaddemo.fetch.fetchcore.server.FetchFileResourceTransporter
import com.downloaddemo.fetch.fetchcore.server.FileRequest
import com.downloaddemo.fetch.fetchcore.server.FileResourceTransporter
import com.downloaddemo.fetch.fetchcore.server.FileResponse
import com.downloaddemo.fetch.fetchcore.server.FileResponse.CREATOR.CLOSE_CONNECTION
import com.downloaddemo.fetch.fetchcore.server.FileResponse.CREATOR.OPEN_CONNECTION
import com.downloaddemo.fetch.fetchfileserver.FileResolver
...
class FetchFileResourceProvider(
            ...
            try {
                while (persistConnection && !interrupted) {
                    ...
                    if (request != null && !interrupted) {
                        ...
                        if (!interrupted && fileResourceProviderDelegate.acceptAuthorization(
                                sessionId, request.authorization, request
                            )
                        ) {
                            ...
                            if (request.extras.isNotEmpty()) {
                                fileResourceProviderDelegate.onClientDidProvideExtras(
                                    sessionId, request.extras, request
                                )
                            }
                            when (request.type) {
                                ...
                                FileRequest.TYPE_CATALOG -> {
                                    if (!interrupted) {
                                        val catalog = fileResourceProviderDelegate.getCatalog(
                                            request.page, request.size
                                        )
                                        val data = catalog.toByteArray(Charsets.UTF_8)
                                        if (!interrupted) {
                                            val contentLength =
                                                (if (request.rangeEnd == -1L) data.size.toLong() else request.rangeEnd) - request.rangeStart
                                            sendCatalogResponse(contentLength, getMd5String(data))
                                            transporter.sendRawBytes(
                                                data,
                                                request.rangeStart.toInt(),
                                                contentLength.toInt()
                                            )
                                        }
                                    }
                                ...
                                FileRequest.TYPE_FILE -> {
                                    val fileResource =
                                        fileResourceProviderDelegate.getFileResource(request.fileResourceId)
                                    if (!interrupted) {
                                        if (fileResource != null) {
                                            this.fileResource = fileResource
                                            inputResourceWrapper =
                                                fileResourceProviderDelegate.getFileInputResourceWrapper(
                                                    sessionId,
                                                    request,
                                                    fileResource,
                                                    request.rangeStart
                                            ...
                                            if (inputResourceWrapper == null) {
                                                if (fileResource.id == FileRequest.CATALOG_ID) {
                                                    val catalog =
                                                        fileResource.extras.getString("data", "{}")
                                                            .toByteArray(Charsets.UTF_8)
                                                    fileResource.length =
                                                        if (request.rangeEnd == -1L) catalog.size.toLong() else request.rangeEnd
                                                    fileResource.md5 = getMd5String(catalog)
                                                    inputResourceWrapper =
                                                        fileResolver.getCatalogInputWrapper(
                                                            catalog, request, fileResource
                                                        )
                                                } else {
                                                    inputResourceWrapper =
                                                        fileResolver.getInputWrapper(fileResource)
                                            ...
                                            if (!interrupted) {
                                                ...
                                                val byteArray =
                                                    ByteArray(FileResourceTransporter.BUFFER_SIZE)
                                                val contentLength =
                                                    (if (request.rangeEnd == -1L) fileResource.length else request.rangeEnd) - request.rangeStart
                                                var remainderBytes = contentLength
                                                sendFileResourceResponse(
                                                    contentLength, fileResource.md5
                                                )
                                                var reportingStartTime = System.nanoTime()
                                                var read =
                                                    inputResourceWrapper?.read(byteArray) ?: -1
                                                var streamBytes: Int
                                                fileResourceProviderDelegate.onStarted(
                                                    sessionId, request, fileResource
                                                )
                                                while (remainderBytes > 0L && read != -1 && !interrupted) {
                                                    ...
                                                    transporter.sendRawBytes(
                                                        byteArray, 0, streamBytes
                                                    )
                                                    if (read != -1) {
                                                        remainderBytes -= streamBytes
                                                        reportingStopTime = System.nanoTime()
                                                        val hasReportingTimeElapsed =
                                                            hasIntervalTimeElapsed(
                                                                reportingStartTime,
                                                                reportingStopTime,
                                                                progressReportingInMillis
                                                            )
                                                        if (hasReportingTimeElapsed && !interrupted) {
                                                            val progress = calculateProgress(
                                                                contentLength - remainderBytes,
                                                                contentLength
                                                            )
                                                            fileResourceProviderDelegate.onProgress(
                                                                sessionId,
                                                                request,
                                                                fileResource,
                                                                progress
                                                        ...
                                                        read = inputResourceWrapper?.read(byteArray)
                                                            ?: -1
                                                    }
                                                }
                                                if (remainderBytes == 0L && !interrupted) {
                                                    fileResourceProviderDelegate.onProgress(
                                                        sessionId, request, fileResource, 100
                                                    )
                                                    fileResourceProviderDelegate.onComplete(
                                                        sessionId, request, fileResource
                                                    )
                                                }
                                            }
                                ...
                                FileRequest.TYPE_INVALID -> {
                                    ...
                                    if (!interrupted) {
                                        fileResourceProviderDelegate.onCustomRequest(
                                            sessionId, request, transporter, interruptMonitor
                                        )
                                    }
                                }
                            }
