...
import com.downloaddemo.fetch.fetchcore.ENQUEUED_REQUESTS_ARE_NOT_DISTINCT
...
import com.downloaddemo.fetch.fetchcore.Func
import com.downloaddemo.fetch.fetchcore.Func2
import com.downloaddemo.fetch.fetchcore.HandlerWrapper
...
import com.downloaddemo.fetch.fetchmain.fetch.FetchModulesBuilder.Modules
import com.downloaddemo.fetch.fetchmain.getErrorFromMessage
import com.downloaddemo.fetch.fetchmain.util.ActiveDownloadInfo
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_ENABLE_LISTENER_AUTOSTART_ON_ATTACHED
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_ENABLE_LISTENER_NOTIFY_ON_ATTACHED
...
open class FetchImpl constructor(
    ...
    private fun registerActiveDownloadsRunnable() {
        handlerWrapper.postDelayed(
            activeDownloadsRunnable, fetchConfiguration.activeDownloadsCheckInterval
        )
    }
    ...
    private fun enqueueRequest(
        requests: List<Request>, func: Func<List<Pair<Request, Error>>>?, func2: Func<Error>?
    ) {
        synchronized(lock) {
    ...
    private fun pauseDownloads(
        ids: List<Int>?, groupId: Int?, func: Func<List<Download>>?, func2: Func<Error>?
    ) {
        synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun removeAllWithStatus(
        status: Status, func: Func<List<Download>>?, func2: Func<Error>?
    ): Fetch {
    ...
    override fun removeAllInGroupWithStatus(
        id: Int, statuses: List<Status>, func: Func<List<Download>>?, func2: Func<Error>?
    ): Fetch {
        return executeRemoveAction(
            { fetchHandler.removeAllInGroupWithStatus(id, statuses) }, func, func2
        )
    }
    ...
    private fun executeRemoveAction(
        downloadAction: () -> List<Download>, func: Func<List<Download>>?, func2: Func<Error>?
    ): Fetch {
        return synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun deleteAllInGroupWithStatus(
        ...
        return executeDeleteAction(
            { fetchHandler.deleteAllInGroupWithStatus(id, statuses) }, func, func2
        )
    }
    ...
    override fun resetAutoRetryAttempts(
        downloadId: Int, retryDownload: Boolean, func: Func2<Download?>?, func2: Func<Error>?
    ): Fetch {
        return synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun renameCompletedDownloadFile(
        id: Int, newFileName: String, func: Func<Download>?, func2: Func<Error>?
    ): Fetch {
        return synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun replaceExtras(
        id: Int, extras: Extras, func: Func<Download>?, func2: Func<Error>?
    ): Fetch {
        return synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun getDownloadsInGroupWithStatus(
        groupId: Int, statuses: List<Status>, func: Func<List<Download>>
    ): Fetch {
        synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun getDownloadsByRequestIdentifier(
        identifier: Long, func: Func<List<Download>>
    ): Fetch {
        synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
    ...
    override fun getContentLengthForRequest(
        request: Request, fromServer: Boolean, func: Func<Long>, func2: Func<Error>?
    ): Fetch {
        synchronized(lock) {
            throwExceptionIfClosed()
    ...
    override fun getFetchFileServerCatalog(
        request: Request, func: Func<List<FileResource>>, func2: Func<Error>?
    ): Fetch {
        synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.executeWorkerTask {
        ...
        synchronized(lock) {
            ...
            handlerWrapper.post {
                try {
                    ...
                    logger.e(
                        "exception occurred whiles shutting down Fetch with namespace:$namespace", e
                    )
                }
            }
        }
    ...
    override fun awaitFinishOrTimeout(allowTimeInMilliseconds: Long) {
        com.downloaddemo.fetch.fetchmain.util.awaitFinishOrTimeout(
            allowTimeInMilliseconds,
            fetchHandler
        )
    }
    ...
    override fun addActiveDownloadsObserver(
        includeAddedDownloads: Boolean, fetchObserver: FetchObserver<Boolean>
    ): Fetch {
        synchronized(lock) {
            throwExceptionIfClosed()
            handlerWrapper.post {
