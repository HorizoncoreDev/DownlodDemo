...
import com.downloaddemo.fetch.fetchcore.getUniqueIdForFileNotFound
...
open class Request constructor(
    ...
    constructor(
         ...
         * downloaded to and saved on disk.*/
        fileUri: Uri, isFileNotFoundError: Int
    ) : this(url, fileUri.toString(), isFileNotFoundError)

    /** Unique Identifier. Used to identify a download.*/
    var id: Int = if (isFileNotFoundError == 1) {
        getUniqueIdForFileNotFound(url, file)
    } else {
        getUniqueId(url, file)
    }
    ...
    override fun toString(): String {
        return "Request(url='$url', file='$file', id=$id, groupId=$groupId, headers=$headers, priority=$priority, networkType=$networkType, tag=$tag)"
    }
    ...
    companion object CREATOR : Parcelable.Creator<Request> {
        ...
        override fun createFromParcel(input: Parcel): Request {
            ...
            val request = Request(url, file, isError)
