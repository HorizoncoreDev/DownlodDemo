...
import com.downloaddemo.fetch.fetchcore.DEFAULT_BUFFER_SIZE
...
import com.downloaddemo.fetch.fetchcore.FETCH_FILE_SERVER_INVALID_RESPONSE_TYPE
...
import com.downloaddemo.fetch.fetchcore.copyStreamToString
import com.downloaddemo.fetch.fetchcore.getFetchFileServerHostAddress
import com.downloaddemo.fetch.fetchcore.getFetchFileServerPort
import com.downloaddemo.fetch.fetchcore.getFileMd5String
import com.downloaddemo.fetch.fetchcore.getFileResourceIdFromUrl
import com.downloaddemo.fetch.fetchcore.getRangeForFetchFileServerRequest
import com.downloaddemo.fetch.fetchcore.getRequestContentLength
import com.downloaddemo.fetch.fetchcore.getRequestSupportedFileDownloaderTypes
...
import com.downloaddemo.fetch.fetchcore.server.FileRequest.CREATOR.TYPE_FILE
 ...
 * @see {@link com.downloaddemo.fetch.fetchcore.Downloader}
 * */
open class FetchFileServerDownloader @JvmOverloads constructor(
    ...
    private val connections: MutableMap<Downloader.Response, FetchFileResourceTransporter> =
        Collections.synchronizedMap(HashMap())

    override fun onPreClientExecute(
        client: FetchFileResourceTransporter, request: Downloader.ServerRequest
    ...
    override fun execute(
        request: Downloader.ServerRequest, interruptMonitor: InterruptMonitor
    ): Downloader.Response? {
        ...
        while (!interruptMonitor.isInterrupted) {
            ...
            val isSuccessful =
                serverResponse.connection == FileResponse.OPEN_CONNECTION && serverResponse.type == TYPE_FILE && serverResponse.status == HttpsURLConnection.HTTP_PARTIAL
            ...
            val acceptsRanges =
                code == HttpsURLConnection.HTTP_PARTIAL || responseHeaders["Accept-Ranges"]?.firstOrNull() == "bytes"
    ...
    override fun onServerResponse(
        request: Downloader.ServerRequest, response: Downloader.Response
    ) {
    ...
    override fun getFetchFileServerCatalog(serverRequest: Downloader.ServerRequest): List<FileResource> {
        ...
        if (response?.byteStream != null) {
            try {
                val type =
                    response.responseHeaders[FileRequest.FIELD_TYPE]?.firstOrNull()?.toInt() ?: -1
                if (type != TYPE_FILE) {
                    disconnect(response)
