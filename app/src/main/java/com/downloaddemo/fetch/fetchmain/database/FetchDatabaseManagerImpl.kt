...
class FetchDatabaseManagerImpl constructor(
    ...
    override fun updateFileBytesInfoAndStatusOnly(downloadInfo: DownloadInfo) {
        ...
        try {
            ...
            database.execSQL(
                "UPDATE ${DownloadDatabase.TABLE_NAME} SET " + "${DownloadDatabase.COLUMN_DOWNLOADED} = ?, " + "${DownloadDatabase.COLUMN_TOTAL} = ?, " + "${DownloadDatabase.COLUMN_STATUS} = ? " + "WHERE ${DownloadDatabase.COLUMN_ID} = ?",
                arrayOf(
    ...
    override fun updateExtras(id: Int, extras: Extras): DownloadInfo? {
        ...
        database.beginTransaction()
        database.execSQL(
            "UPDATE ${DownloadDatabase.TABLE_NAME} SET " + "${DownloadDatabase.COLUMN_EXTRAS} = ? " + "WHERE ${DownloadDatabase.COLUMN_ID} = ?",
    ...
    private val pendingCountQuery =
        "SELECT ${DownloadDatabase.COLUMN_ID} FROM ${DownloadDatabase.TABLE_NAME}" + " WHERE ${DownloadDatabase.COLUMN_STATUS} = '${Status.QUEUED.value}'" + " OR ${DownloadDatabase.COLUMN_STATUS} = '${Status.DOWNLOADING.value}'"

    private val pendingCountIncludeAddedQuery =
        "SELECT ${DownloadDatabase.COLUMN_ID} FROM ${DownloadDatabase.TABLE_NAME}" + " WHERE ${DownloadDatabase.COLUMN_STATUS} = '${Status.QUEUED.value}'" + " OR ${DownloadDatabase.COLUMN_STATUS} = '${Status.DOWNLOADING.value}'" + " OR ${DownloadDatabase.COLUMN_STATUS} = '${Status.ADDED.value}'"
    ...
    private fun sanitize(downloads: List<DownloadInfo>, firstEntry: Boolean = false): Boolean {
        ...
        for (element in downloads) {
            ...
            when (downloadInfo.status) {
                ...
                Status.QUEUED, Status.PAUSED -> onPaused(downloadInfo)
                Status.CANCELLED, Status.FAILED, Status.ADDED, Status.NONE, Status.DELETED, Status.REMOVED -> {
                }
            }
        }
