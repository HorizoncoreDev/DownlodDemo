...
import com.downloaddemo.fetch.fetchmain.Priority
import com.downloaddemo.fetch.fetchmain.Status
import com.downloaddemo.fetch.fetchmain.util.defaultPriority
...
import com.downloaddemo.fetch.fetchmain.*
import com.downloaddemo.fetch.fetchmain.util.*
import java.util.*


@Entity(
    tableName = DownloadDatabase.TABLE_NAME,
    indices = [(Index(value = [DownloadDatabase.COLUMN_FILE], unique = true)), (Index(
        value = [DownloadDatabase.COLUMN_GROUP, DownloadDatabase.COLUMN_STATUS], unique = false
    ))]
)
open class DownloadInfo : Download {
    ...
    @ColumnInfo(
        name = DownloadDatabase.COLUMN_DOWNLOAD_ON_ENQUEUE, typeAffinity = ColumnInfo.INTEGER
    )
    ...
    @ColumnInfo(
        name = DownloadDatabase.COLUMN_AUTO_RETRY_MAX_ATTEMPTS, typeAffinity = ColumnInfo.INTEGER
    )
    override var autoRetryMaxAttempts: Int = DEFAULT_AUTO_RETRY_ATTEMPTS

    @ColumnInfo(
        name = DownloadDatabase.COLUMN_AUTO_RETRY_ATTEMPTS, typeAffinity = ColumnInfo.INTEGER
    )
    ...
    override val request: Request
        get() {
            val request = Request(url, file,0)
    ...
    override fun toString(): String {
        return "DownloadInfo(id=$id, namespace='$namespace', url='$url', file='$file', " + "group=$group, priority=$priority, headers=$headers, downloaded=$downloaded," + " total=$total, status=$status, error=$error, networkType=$networkType, " + "created=$created, tag=$tag, enqueueAction=$enqueueAction, identifier=$identifier," + " downloadOnEnqueue=$downloadOnEnqueue, extras=$extras, " + "autoRetryMaxAttempts=$autoRetryMaxAttempts, autoRetryAttempts=$autoRetryAttempts," + " etaInMilliSeconds=$etaInMilliSeconds, downloadedBytesPerSecond=$downloadedBytesPerSecond)"
    }
