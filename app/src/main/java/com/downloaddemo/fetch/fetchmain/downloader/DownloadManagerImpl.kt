...
import com.downloaddemo.fetch.fetchmain.ACTION_QUEUE_BACKOFF_RESET
...
import com.downloaddemo.fetch.fetchmain.fetch.ListenerCoordinator
    ...
    override fun start(download: Download): Boolean {
        return synchronized(lock) {
            ...
            if (downloadCounter >= concurrentLimit) {
                logger.d(
                    "DownloadManager cannot init download $download because " + "the download queue is full"
                )
                return false
            }
            downloadCounter += 1
