...
import com.downloaddemo.fetch.fetchmain.DownloadNotification.ActionType.*
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_NOTIFICATION_TIMEOUT_AFTER
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_NOTIFICATION_TIMEOUT_AFTER_RESET
import com.downloaddemo.fetch.fetchmain.util.onDownloadNotificationActionTriggered
...
abstract class DefaultFetchNotificationManager(context: Context) : FetchNotificationManager {
        ...
        get() = object : BroadcastReceiver() {
            ...
            override fun onReceive(context: Context?, intent: Intent?) {
                onDownloadNotificationActionTriggered(
                    context, intent, this@DefaultFetchNotificationManager
                )
            }
    ...
    override fun updateGroupSummaryNotification(
        ...
        notificationBuilder.setPriority(NotificationCompat.PRIORITY_DEFAULT)
            ...
            .setContentText("").setStyle(style).setOnlyAlertOnce(true).setGroup(groupId.toString())
    ...
    override fun updateNotification(
        ...
        notificationBuilder.setPriority(NotificationCompat.PRIORITY_DEFAULT).setSmallIcon(smallIcon)
            ...
            .setGroup(downloadNotification.groupId.toString()).setGroupSummary(false)
        ...
        when {
            downloadNotification.isDownloading -> {
                notificationBuilder.setTimeoutAfter(getNotificationTimeOutMillis()).addAction(
    ...
    override fun getActionPendingIntent(
        downloadNotification: DownloadNotification, actionType: DownloadNotification.ActionType
    ): PendingIntent {
    ...
    override fun getGroupActionPendingIntent(
        ...
        synchronized(downloadNotificationsMap) {
            ...
            intent.putExtra(EXTRA_ACTION_TYPE, action)
            return PendingIntent.getBroadcast(
                context, groupId + action, intent, PendingIntent.FLAG_UPDATE_CURRENT
            )
        }
    }
    ...
    override fun notify(groupId: Int) {
        synchronized(downloadNotificationsMap) {
            ...
            val useGroupNotification = updateGroupSummaryNotification(
                groupId, groupSummaryNotificationBuilder, groupedDownloadNotifications, context
            )
    ...
    override fun postDownloadUpdate(download: Download): Boolean {
        return synchronized(downloadNotificationsMap) {
            ...
            downloadNotificationsMap[download.id] = downloadNotification
            if (downloadNotificationExcludeSet.contains(downloadNotification.notificationId) && !downloadNotification.isFailed && !downloadNotification.isCompleted) {
                downloadNotificationExcludeSet.remove(downloadNotification.notificationId)
            }
    ...
    override fun getNotificationBuilder(
        notificationId: Int, groupId: Int
    ): NotificationCompat.Builder {
        synchronized(downloadNotificationsMap) {
            val notificationBuilder =
                downloadNotificationsBuilderMap[notificationId] ?: NotificationCompat.Builder(
                    context, getChannelId(notificationId, context)
                )
            downloadNotificationsBuilderMap[notificationId] = notificationBuilder
            notificationBuilder.setGroup(notificationId.toString()).setStyle(null)
                .setProgress(0, 0, false).setContentTitle(null).setContentText(null)
                .setContentIntent(null).setGroupSummary(false)
                .setTimeoutAfter(DEFAULT_NOTIFICATION_TIMEOUT_AFTER_RESET).setOngoing(false)
                .setGroup(groupId.toString()).setOnlyAlertOnce(true)
                .setSmallIcon(android.R.drawable.stat_sys_download_done).mActions.clear()
    ...
    override fun getSubtitleText(
        context: Context, downloadNotification: DownloadNotification
    ): String {
        return when {
    ...
    private fun getEtaText(context: Context, etaInMilliSeconds: Long): String {
        ...
        return when {
            hours > 0 -> context.getString(
                R.string.fetch_notification_download_eta_hrs, hours, minutes, seconds
            )

            minutes > 0 -> context.getString(
                R.string.fetch_notification_download_eta_min, minutes, seconds
            )
