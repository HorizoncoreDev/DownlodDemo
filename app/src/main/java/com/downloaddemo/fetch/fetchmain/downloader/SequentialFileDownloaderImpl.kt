...
import com.downloaddemo.fetch.fetchcore.GET_REQUEST_METHOD
...
import com.downloaddemo.fetch.fetchcore.getFileUri
...
class SequentialFileDownloaderImpl(
        ...
        try {
            ...
            if (!interrupted && !terminated) {
                ...
                if (!interrupted && !terminated && response != null && isResponseSuccessful) {
                    downloaded =
                        if (response.code == HttpsURLConnection.HTTP_PARTIAL || response.acceptsRanges) {
                            initialDownload.downloaded
                        } else {
                            0
                        }
                    total = response.contentLength
                    val seekPosition = if (response.code == HttpsURLConnection.HTTP_PARTIAL) {
    ...
    private fun writeToOutput(
        input: BufferedInputStream, outputResourceWrapper: OutputResourceWrapper?, bufferSize: Int
    ) {
