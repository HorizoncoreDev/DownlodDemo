...
fun getErrorFromMessage(message: String?): Error {
    return if (message == null || message.isEmpty()) {
        Error.UNKNOWN
    } else if (message.equals(REQUEST_WITH_FILE_PATH_ALREADY_EXIST, true) || message.contains(
        ...
        Error.FILE_NOT_CREATED
            ...
            ETIMEDOUT,
            ignoreCase = true
        ) || message.contains(CONNECTION_TIMEOUT, ignoreCase = true) || message.contains(
    ...
    } else if (message.equals(ENOSPC, ignoreCase = true) || message.equals(
            DATABASE_DISK_FULL,
            ignoreCase = true
        )
    ) {
