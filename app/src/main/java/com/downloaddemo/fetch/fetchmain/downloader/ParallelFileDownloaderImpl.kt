...
import com.downloaddemo.fetch.fetchcore.AverageCalculator
import com.downloaddemo.fetch.fetchcore.DOWNLOAD_INCOMPLETE
import com.downloaddemo.fetch.fetchcore.DownloadBlockInfo
import com.downloaddemo.fetch.fetchcore.Downloader
import com.downloaddemo.fetch.fetchcore.EMPTY_RESPONSE_BODY
import com.downloaddemo.fetch.fetchcore.FileSlice
import com.downloaddemo.fetch.fetchcore.FileSliceInfo
import com.downloaddemo.fetch.fetchcore.HEAD_REQUEST_METHOD
import com.downloaddemo.fetch.fetchcore.INVALID_CONTENT_HASH
import com.downloaddemo.fetch.fetchcore.InterruptMonitor
import com.downloaddemo.fetch.fetchcore.Logger
import com.downloaddemo.fetch.fetchcore.OutputResourceWrapper
import com.downloaddemo.fetch.fetchcore.RESPONSE_NOT_SUCCESSFUL
import com.downloaddemo.fetch.fetchcore.StorageResolver
import com.downloaddemo.fetch.fetchcore.UNKNOWN_ERROR
import com.downloaddemo.fetch.fetchcore.calculateEstimatedTimeRemainingInMilliseconds
import com.downloaddemo.fetch.fetchcore.copyDownloadResponseNoStream
import com.downloaddemo.fetch.fetchcore.getFile
import com.downloaddemo.fetch.fetchcore.hasIntervalTimeElapsed
...
import com.downloaddemo.fetch.fetchmain.exception.FetchException
import com.downloaddemo.fetch.fetchmain.getErrorFromThrowable
import com.downloaddemo.fetch.fetchmain.helper.FileDownloaderDelegate
import com.downloaddemo.fetch.fetchmain.provider.NetworkInfoProvider
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_DOWNLOAD_SPEED_REPORTING_INTERVAL_IN_MILLISECONDS
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_FILE_SLICE_NO_LIMIT_SET
import com.downloaddemo.fetch.fetchmain.util.deleteAllInFolderForId
import com.downloaddemo.fetch.fetchmain.util.getDownloadedInfoFilePath
import com.downloaddemo.fetch.fetchmain.util.getFileSliceInfo
import com.downloaddemo.fetch.fetchmain.util.getPreviousSliceCount
import com.downloaddemo.fetch.fetchmain.util.getRequestForDownload
import com.downloaddemo.fetch.fetchmain.util.getSavedDownloadedInfo
import com.downloaddemo.fetch.fetchmain.util.saveCurrentSliceCount
import com.downloaddemo.fetch.fetchmain.util.toDownloadInfo
...
class ParallelFileDownloaderImpl(
                        ...
                        if (!interrupted && !terminated) {
                            ...
                            downloadBlocks.forEach { downloadBlock ->
                                delegate?.onDownloadBlockUpdated(
                                    downloadInfo, downloadBlock, totalDownloadBlocks
                                )
                            }
                        }
            ...
            if (!isDownloadComplete() && !terminated && !interrupted) {
                ...
                if (hashCheckingEnabled) {
                    if (openingResponse != null && downloader.verifyContentHash(
                            openingResponse.request, openingResponse.hash
                        )
                    ) {
    ...
    private fun getFileSliceList(
        acceptsRanges: Boolean, request: Downloader.ServerRequest
    ): List<FileSlice> {
    ...
    private fun getChuckInfo(request: Downloader.ServerRequest): FileSliceInfo {
        val fileSliceSize =
            downloader.getFileSlicingCount(request, total) ?: DEFAULT_FILE_SLICE_NO_LIMIT_SET
    ...
    private fun waitAndPerformProgressReporting() {
        ...
        while (actionsCounter != actionsTotal && !interrupted && !terminated) {
            ...
            val downloadSpeedCheckTimeElapsed = hasIntervalTimeElapsed(
                ...
                downloadSpeedStopTime,
                DEFAULT_DOWNLOAD_SPEED_REPORTING_INTERVAL_IN_MILLISECONDS
            )
            ...
            val hasReportingTimeElapsed = hasIntervalTimeElapsed(
                reportingStartTime, reportingStopTime, progressReportingIntervalMillis
            )
    ...
    private fun downloadSliceFiles(
        request: Downloader.ServerRequest, fileSlicesDownloadsList: List<FileSlice>
    ) {
        actionsCounter = 0
        actionsTotal = fileSlicesDownloadsList.size
        if (!storageResolver.fileExists(request.file)) {
            storageResolver.createFile(
                request.file, initialDownload.enqueueAction == EnqueueAction.INCREMENT_FILE_NAME
            )
        }
                    ...
                    try {
                        val file = getFile(
                            getDownloadedInfoFilePath(
                                fileSlice.id, fileSlice.position, fileTempDir
                            )
                        )
                        ...
                        if (!terminated && !interrupted && downloadResponse?.isSuccessful == true) {
                            ...
                            var read: Int =
                                downloadResponse.byteStream?.read(buffer, 0, bufferSize) ?: -1
