...
open class HttpUrlConnectionDownloader @JvmOverloads constructor(
...
) : Downloader<HttpsURLConnection, Void> {
    ...
    private val connections: MutableMap<Downloader.Response, HttpsURLConnection> =
        Collections.synchronizedMap(HashMap<Downloader.Response, HttpsURLConnection>())
    private val cookieManager = getDefaultCookieManager()

    override fun onPreClientExecute(
        client: HttpsURLConnection, request: Downloader.ServerRequest
    ): Void? {
    ...
    override fun execute(
        ...
        var client = httpUrl.openConnection() as HttpsURLConnection
        ...
        if ((code == HttpsURLConnection.HTTP_MOVED_TEMP || code == HttpsURLConnection.HTTP_MOVED_PERM || code == HttpsURLConnection.HTTP_SEE_OTHER) && getHeaderValue(
            ...
            client = httpUrl.openConnection() as HttpsURLConnection
            onPreClientExecute(client, request)
            if (client.getRequestProperty("Referer") == null) {
                val referer = getRefererFromUrl(request.url)
                client.addRequestProperty("Referer", referer)
    ...
    private fun disconnectClient(client: HttpsURLConnection?) {
        try {
            client?.disconnect()
        } catch (e: Exception) {
            e.printStackTrace()
