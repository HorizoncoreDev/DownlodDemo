...
import com.downloaddemo.fetch.fetchmain.FetchGroupListener
import com.downloaddemo.fetch.fetchmain.FetchListener
import com.downloaddemo.fetch.fetchmain.FetchNotificationManager
import com.downloaddemo.fetch.fetchmain.provider.DownloadProvider
            ...
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_ADDED
                    )
                }
        ...
        override fun onQueued(download: Download, waitingOnNetwork: Boolean) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (fetchListener == null) {
                                ...
                                fetchListener.onQueued(
                                    groupId, download, waitingOnNetwork, fetchGroup
                                )
                            }
                        }
                    }
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_QUEUED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
            ...
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    ...
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_WAITING_ON_NETWORK
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_WAITING_ON_NETWORK
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onCompleted(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_COMPLETED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_COMPLETED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onError(download: Download, error: Error, throwable: Throwable?) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (fetchListener == null) {
                                ...
                                fetchListener.onError(
                                    groupId, download, error, throwable, fetchGroup
                                )
                            }
                        }
                    }
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_ERROR
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onDownloadBlockUpdated(
            ...
            synchronized(lock) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (fetchListener == null) {
                            ...
                            fetchListener.onDownloadBlockUpdated(
                                download, downloadBlock, totalBlocks
                            )
                        }
                    }
                }
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_BLOCK_UPDATED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                            ...
                            if (fetchListener == null) {
                                ...
                                fetchListener.onDownloadBlockUpdated(
                                    groupId, download, downloadBlock, totalBlocks, fetchGroup
                                )
                            }
                        }
                    }
        ...
        override fun onStarted(
            ...
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_STARTED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                            ...
                            if (fetchListener == null) {
                                ...
                                fetchListener.onStarted(
                                    groupId, download, downloadBlocks, totalBlocks, fetchGroup
                                )
                            }
                        }
                    }
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_STARTED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onProgress(
            ...
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_PROGRESS_CHANGED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_PROGRESS_CHANGED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onPaused(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_PAUSED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onResumed(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_RESUMED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_RESUMED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onCancelled(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_CANCELLED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_CANCELLED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onRemoved(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_REMOVED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_REMOVED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
        ...
        override fun onDeleted(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId, download, Reason.DOWNLOAD_DELETED
                    )
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group, download, Reason.DOWNLOAD_DELETED
                    )
                }
                val downloadObserverSet = downloadsObserverMap[download.id]
                downloadObserverSet?.forEach {
