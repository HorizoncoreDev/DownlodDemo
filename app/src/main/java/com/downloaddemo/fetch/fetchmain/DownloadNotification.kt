...
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_INSTANCE_NAMESPACE
...
open class DownloadNotification : Parcelable, Serializable {
    ...
    val isCancelledNotification: Boolean
        get() {
            return when (status) {
                Status.DELETED, Status.REMOVED, Status.CANCELLED -> true
                else -> false
            }
        }
    ...
    val isRemovableNotification: Boolean
        get() {
            return when (status) {
                Status.COMPLETED, Status.FAILED, Status.CANCELLED, Status.REMOVED, Status.DELETED -> true
                else -> false
            }
        }
    ...
    override fun toString(): String {
        return "DownloadNotification(status=$status, progress=$progress, notificationId=$notificationId," + " groupId=$groupId, etaInMilliSeconds=$etaInMilliSeconds, downloadedBytesPerSecond=$downloadedBytesPerSecond, " + "total=$total, downloaded=$downloaded, namespace='$namespace', title='$title')"
    }
