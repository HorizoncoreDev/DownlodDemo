...
import com.downloaddemo.fetch.fetchmain.util.DEFAULT_PRIORITY_QUEUE_INTERVAL_IN_MILLISECONDS
...
class PriorityListProcessorImpl constructor(
    ...
    @Volatile override var downloadConcurrentLimit: Int,
    ...
    private val networkChangeListener: NetworkInfoProvider.NetworkChangeListener =
        object : NetworkInfoProvider.NetworkChangeListener {
            override fun onNetworkChanged() {
                handlerWrapper.post {
                    if (!stopped && !paused && networkInfoProvider.isNetworkAvailable && backOffTime > DEFAULT_PRIORITY_QUEUE_INTERVAL_IN_MILLISECONDS) {
                        resetBackOffTime()
                    }
                }
            }
    ...
    init {
        ...
        context.registerReceiver(
            priorityBackoffResetReceiver, IntentFilter(ACTION_QUEUE_BACKOFF_RESET)
        )
    }
