package com.downloaddemo.fetch.fetchokhttp
...
import com.downloaddemo.fetch.fetchcore.acceptRanges
import com.downloaddemo.fetch.fetchcore.copyStreamToString
import com.downloaddemo.fetch.fetchcore.getContentLengthFromHeader
import com.downloaddemo.fetch.fetchcore.getFileMd5String
import com.downloaddemo.fetch.fetchcore.getHeaderValue
import com.downloaddemo.fetch.fetchcore.getRefererFromUrl
...
open class OkHttpDownloader @JvmOverloads constructor(
    ...
    private val connections: MutableMap<Downloader.Response, Response> =
        Collections.synchronizedMap(HashMap())

    @Volatile
    var client: OkHttpClient =
        okHttpClient ?: OkHttpClient.Builder().readTimeout(20_000L, TimeUnit.MILLISECONDS)
            .connectTimeout(15_000L, TimeUnit.MILLISECONDS).cache(null).followRedirects(true)
            .followSslRedirects(true).retryOnConnectionFailure(false)
//            .cookieJar(getDefaultCookieJar())
            .build()

    override fun onPreClientExecute(
        client: OkHttpClient, request: Downloader.ServerRequest
    ): Request {
        val okHttpRequestBuilder =
            Request.Builder().url(request.url).method(request.requestMethod, null)
    ...
    private fun getRedirectedServerRequest(
        oldRequest: Downloader.ServerRequest, redirectUrl: String
    ...
    override fun execute(
        ...
        if ((code == HttpURLConnection.HTTP_MOVED_TEMP || code == HttpURLConnection.HTTP_MOVED_PERM || code == HttpURLConnection.HTTP_SEE_OTHER) && getHeaderValue(
                responseHeaders, "Location"
            ) != null
        ) {
            okHttpRequest = onPreClientExecute(
                client, getRedirectedServerRequest(
                    request, getHeaderValue(responseHeaders, "Location") ?: ""
                )
            )
            if (okHttpRequest.header("Referer") == null) {
                val referer = getRefererFromUrl(request.url)
